...
for i in range(n_im):
    ...
    Mr = cv2.resize(M, None, fx=1.0/rescaling_factor, fy=1.0/rescaling_factor, interpolation=cv2.INTER_NEAREST) > 128
    ...
    F[ind_p + npix_per_image_and_class : ind_p + 2 * npix_per_image_and_class,:] = feats[idx_neg, :]

    t[ind_p : ind_p + npix_per_image_and_class] = 1
    t[ind_p + npix_per_image_and_class + 1: ind_p + 2 * npix_per_image_and_class] = 0
...
clf = make_pipeline(StandardScaler(), SVC(kernel='linear', verbose=True))
clf.fit(F, t)
