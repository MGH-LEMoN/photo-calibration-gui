...
def registration(centers, radii, true_width, true_height):
    ...
    if False:  # Matching, one corner at the time (not a good idea, see below)
        ...
        for c in range(4):
            ...
            kp_tmp = []
            des_tmp = np.zeros(shape=(0, des_template.shape[1]),
                               dtype='float32')
            ...
            template_pts = np.float32([kp_tmp[m.queryIdx].pt
                                       for m in matches]).reshape(-1, 1, 2)
            target_pts = np.float32(
                [kp_target[m.trainIdx].pt for m in matches]).reshape(-1, 1, 2)

            # Fit transform and apply to corner
            M, _ = cv2.findHomography(template_pts, target_pts, cv2.RANSAC,
                                      5.0)

            centers_target[c, :] = cv2.perspectiveTransform(
                centers[c, :].reshape(-1, 1, 2), M)
